...
import { ElementTypeWithId } from "@src/elements/types";

const myRoutine: number[] = [0, 48, 139, 48, 217, 60, 207, 160, 244, 249, 273];

export default function PommelHorse() {
  const [routine, setRoutine] = useState<ElementTypeWithId[]>(
    myRoutine.map(
      (index) =>
        ({ ...Elements.pommel_horse[index], id: uuidv4() } as ElementTypeWithId)
    )
  );
  return (
    <>
